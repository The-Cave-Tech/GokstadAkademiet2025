...
export const norwegianPhoneNumberValidation = z
  ...
  .refine(val => {
    ...
    const localNumber = digitsOnly.startsWith('47') ? digitsOnly.slice(2) : digitsOnly;
...
export function formatPhoneNumber(phoneNumber: string): string {
  ...
  if (digitsOnly.length === 8 && !phoneNumber.startsWith('+')) {
    return `+47 ${digitsOnly.slice(0, 3)} ${digitsOnly.slice(3, 5)} ${digitsOnly.slice(5)}`;
  }
  
  if (digitsOnly.startsWith('47') && digitsOnly.length >= 10) {
    const localPart = digitsOnly.slice(2, 10);
    return `+47 ${localPart.slice(0, 3)} ${localPart.slice(3, 5)} ${localPart.slice(5, 8)}`;
  }
  ...
  if (phoneNumber.startsWith('+')) {
    ...
    if (countryCodeMatch) {
      ...
      if (localPart.length === 8) {
        return `+${countryCode} ${localPart.slice(0, 3)} ${localPart.slice(3, 5)} ${localPart.slice(5, 8)}`;
      }
    }
  }
 
  if (!phoneNumber.includes('+')) {
    return `+47 ${digitsOnly.slice(0, 3)} ${digitsOnly.slice(3, 5)} ${digitsOnly.slice(5, 8)}`;
  }
...
export function validateProfileForm<T>(schema: z.ZodType<T>, formData: Partial<T>): ValidationResult {
  try {
