...
export async function register(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  console.log("[Server] Register - Validation result:", { success: validation.success, errors: validation.success ? null : validation.error.flatten().fieldErrors });

  if (!validation.success) {
    const firstErrors = Object.fromEntries(
      Object.entries(validation.error.flatten().fieldErrors).map(([key, errors]) => [key, [errors[0]]]) // Returner feil som en array
    );
